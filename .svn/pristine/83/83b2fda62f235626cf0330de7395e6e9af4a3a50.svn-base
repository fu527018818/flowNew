...
<script>
    ...
    export default {
        name:"userPortrayal",
        components:{MainNav,cardRectangle},
        data(){
            return {
              chart:"",
              colorChange:['11','22'],
        ...
        methods:{
          portrayalInit(){
           ...
           .then(res=>{
               ...
               Highcharts.chart('contain1',userOptionColumn(current_consumption_times.name,current_consumption_times.dateX,current_consumption_times.y,true,'消费次数'))

               var current_consumption_price =formattBg.formatOneBar(data.graphic.consumption_price);
               Highcharts.chart('contain2',userOptionColumn(current_consumption_price.name,current_consumption_price.dateX,current_consumption_price.y,true,'累计消费金额'))

               var current_loss_member =formattBg.formatOneBar(data.graphic.loss_member);
               Highcharts.chart('contain3',userOptionColumn(current_loss_member.name,current_loss_member.dateX,current_loss_member.y,false,'客户流失分布'))

               var current_conversion_rate =formattBg.formatOneBar(data.graphic.conversion_rate);
               Highcharts.chart('contain4',userOptionColumn(current_conversion_rate.name,current_conversion_rate.dateX,current_conversion_rate.y,false,'客户转化率分布'))
               
               var current_average_visit =formattBg.formatOneBar(data.graphic.average_visit);
               Highcharts.chart('contain5',userOptionColumn(current_average_visit.name,current_average_visit.dateX,current_average_visit.y,true,'平均到店周期'))

              var current_age_ratio =formattBg.formatOneBar(data.graphic.age_ratio);
               Highcharts.chart('contain6',userOptionColumn(current_age_ratio.name,current_age_ratio.dateX,current_age_ratio.y,true,'年龄占比'))
