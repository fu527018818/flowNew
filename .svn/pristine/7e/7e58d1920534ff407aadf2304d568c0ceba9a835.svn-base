<template>
    <div> 
        ...
        <indent-table ref="ordertable" :lists="lists"></indent-table>
...
export default {
    name:"searchOrder", //搜索订单
    components: {searchPage,searchCondition,indentTable},
    data(){
      return{
  ...
  methods:{
    ...
    indentInit(val){
        if(this.$refs.ordertable){
            this.$refs.ordertable.loading = true;
        }
       ...
       statisticsIndent({
          ...
          .then(res=>{
              ...
              setTimeout(()=>{if(this.$refs.ordertable){
                    this.$refs.ordertable.loading = false;
                }},500);
    ...
    created(){
        if( this.$route.query.search){
            this.searchOrder = this.$route.query.search;
        }else{
            this.searchOrder = ""
        }
        this.changeDate('今天');
