            ...
            <div class="contentBox_child">
                <div class="content">
                    <div class="dateShowBox">
                          <search-date ref="currentData" @seachtrigger="seachtrigger" :tit="'客流'" :isShowDate="true" :isShowId="false"></search-date>
                    ...
                    <div class="trendChartBox">
                           ...
                           <div id="turnoverCut">
                                <el-radio-group v-model="cut_trend_chart" @change="cutChange">
                                    ...
                                    <el-radio-button label="return_rate">回头客</el-radio-button>
                                    <el-radio-button label="return_member_rate">回头率</el-radio-button>
        ...
        methods:{
           seachtrigger(val){
              let rootDate =this.$route.params;
                if(rootDate.type){
                    this.date = rootDate.date;
                    this.type = rootDate.type;
                    this.changeCardAcross(this.type);
                    this.$refs.currentData.last_visit = this.date;
                }else{
                 this.date = val
                }
               this.passengerFlowInit();
            },
            passengerFlowInit(){
                //默认为今天
                statisticsPassenger({
                ...
                .then(res=>{
                       console.log(res)
                        ...
                        //删除PC端不需要字段
                        // delete data.chart.return_guest;
                        //this.chart = data.chart;
                        //排序
                        this.chart = [];
                        if (data.chart) {
                            data.chart.passenger&&this.chart.push(data.chart.passenger);
                            data.chart.new_guest&&this.chart.push(data.chart.new_guest);
                            data.chart.old_guest&&this.chart.push(data.chart.old_guest);
                            data.chart.return_guest&&this.chart.push(data.chart.return_guest);
                            data.chart.return_member_rate&&this.chart.push(data.chart.return_member_rate);
                            
                            // data.chart.into_shop_rate&&this.chart.push(data.chart.into_shop_rate)进店率暂时没有
                            data.chart.visit_guest&&this.chart.push(data.chart.visit_guest);
                        }
           ...
           formatGraphic(data){ //格式化数据趋势图数据
            ...
            penetrate(item){
                if(item.name=="进店率"||item.name=="访客"){
                    return
                }
                this.$router.push({name:'userManage',params:{type:item.name,date:this.date}})
            },
            changeCardAcross(type){
                switch(type){
                    case '总人流':
                     this.cut_trend_chart = "current_trend";
                    break;
                    case '客流量':
                     this.cut_trend_chart = "passenger";
                    break;
                    case '进店率':
                     this.cut_trend_chart = "into_shop_rate";
                    break;
                    case '新客':
                     this.cut_trend_chart = "new_guest";
                    break;
                    case '老客':
                     this.cut_trend_chart = "old_guest";
                    break;
                    case '回头率':
                     this.cut_trend_chart = "return_rate";
                    break;
                }
            }
        },
...
<style scoped lang="scss">
 ...
 .dataListBox /deep/ .dataList{
         & >div:nth-child(1),& >div:nth-child(2),& >div:nth-child(3),& >div:nth-child(5),& >div:nth-child(6){
             cursor: pointer;
         }
    }
</style>